...
st.markdown("""
### 📥 사용 안내
본 사이트는 서울교통공사에서 제공하는 **‘서울 지하철 호선별·역별·시간대별 승하차 인원 데이터(CSV)’**를 기반으로 동작합니다.

1. 공공데이터포털(data.go.kr)에서  
   **서울교통공사_지하철 호선별 역별 시간대별 승하차 인원 데이터(CSV)**  
   를 다운로드하세요.

2. 다운로드한 CSV 파일을 **ZIP으로 압축(zip)** 해주세요.

3. 아래 업로드 창에 ZIP 파일을 업로드하면  
   📊 **시간대별 승하차 분석 및 그래프 시각화가 자동으로 실행됩니다.**

※ CSV 파일 크기가 커서 GitHub에 직접 포함할 수 없어,  
   **사용자 업로드 방식으로 동작하는 구조**입니다.
""")
